...
def get_args(json_name):
    argparser = argparse.ArgumentParser(description=__doc__)
    argparser.add_argument(
        '-c', '--config',
        metavar='C',
        default=os.path.join('configs', json_name),
...
def get_mata_data(from_data, to_data):
    with open(from_data, 'rb') as f:
        data = pickle.load(f)
    print(len(data))
    # for word in data:
    #     print(word)
    id2word = {index: label for label, index in data.items()}
